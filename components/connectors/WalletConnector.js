...
var WalletConnector = function (_a) {
    ...
    var handleConnect = function () { return __awaiter(void 0, void 0, void 0, function () {
        var mockSignature, mockAddress;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    mockSignature = '0x' + Array(130).fill(0).map(function () { return Math.floor(Math.random() * 16).toString(16); }).join('');
                    mockAddress = '0x1234567890123456789012345678901234567890';
                    return [4 /*yield*/, login(types_1.AuthProviderType.WALLET, {
    ...
    return (<div>
            ...
            <button onClick={handleConnect} disabled={loading} className="w-full flex items-center justify-center p-3 bg-pink-600 rounded-lg hover:bg-pink-700 transition-colors duration-200 text-white font-bold disabled:bg-pink-800 disabled:cursor-not-allowed">
                {loading ? <SpinnerIcon_1.SpinnerIcon /> : 'Sign & Authenticate'}
            </button>
        </div>);
};
